...
public class TriangleFilledInStrategy implements IDrawStrategy{
	...
	public void draw(IShape shape, Graphics2D graphics){  
		...
		int xCorner = x1;              // if x2 > x1, use x1, else use x2

		
		int y1 = (int) shape.getStartPoint().getY();
		int y2 = (int) shape.getEndPoint().getY();
		int yCorner = y2;              // if y2 > y1, use y2, else use y1 as the corner
